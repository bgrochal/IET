/*
 * Taken from: https://github.com/justinethier/AVL-Tree/blob/master/AvlTree.java
 */

package com.github.bgrochal.algorithm.location.separators.lib;
...
// but it needs some attention and does not appear to be 100% correct
    ...
    public AvlNode<T> getFirst() {
        return root == null ? null : getFirst(root);
    }

    private AvlNode<T> getFirst(AvlNode<T> root) {
        if (root.left == null) {
            return root;
        }
    ...
    public AvlNode<T> getLast() {
        return root == null ? null : getLast(root);
    }

    private AvlNode<T> getLast(AvlNode<T> root) {
        if (root.right == null) {
            return root;
        }
    ...
    public AvlNode<TreeNodeData> getNext(AvlNode<TreeNodeData> current) {
        int nextNumber = current.getElement().getSeparatorNumber() + 1;
        return findNodeBySeparatorNumber((AvlNode<TreeNodeData>) root, nextNumber);
    }

    public AvlNode<TreeNodeData> getPrevious(AvlNode<TreeNodeData> current) {
        int previousNumber = current.getElement().getSeparatorNumber() - 1;
        return findNodeBySeparatorNumber((AvlNode<TreeNodeData>) root, previousNumber);
    }

    private AvlNode<TreeNodeData> findNodeBySeparatorNumber(AvlNode<TreeNodeData> root, int number) {
        if (root.getElement().getSeparatorNumber() == number) {
            return root;
        }
        if (root.getElement().getSeparatorNumber() < number) {
            return findNodeBySeparatorNumber(root.getRight(), number);
        }
        if (root.getElement().getSeparatorNumber() > number) {
            return findNodeBySeparatorNumber(root.getLeft(), number);
        }
